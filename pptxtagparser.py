"""
core/pptxtagparser.py

Scans an uploaded .pptx template for <<TAG>> style placeholders inside
textboxes on slide 1 (the first slide is treated as the master layout to
duplicate). Returns which known fields were found, plus how many numbered
speaker slots exist (SPEAKER_NAME_1, SPEAKER_NAME_2, ...).
"""
...
def extract_tags_from_pptx(pptx_path_or_file) -> TemplateTagReport:
    ...
    if max_slot == 0:
        report.warnings.append(
            "No <<SPEAKER_NAME_n>> style tags were found. Add tags like "
            "<<SPEAKER_NAME_1>>, <<SPEAKER_TITLE_1>>, <<SPEAKER_COMPANY_1>>, "
            "<<SPEAKER_PHOTO_1>> for each speaker slot on your template."
        )
    ...
    for slot in range(1, max_slot + 1):
        for prefix in SPEAKER_FIELD_PREFIXES:
            if slot not in speaker_fields_present[prefix]:
                report.warnings.append(
                    f"Speaker slot {slot}: missing tag {prefix}{slot} on the template."
                )
