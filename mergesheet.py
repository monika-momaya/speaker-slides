...
Reads the filled-in Excel merge sheet (generated by core.exceltemplate) and
groups rows into per-slide dicts, using the "Require Single Slide" column to
combine multiple speaker rows onto one slide.
"""
...
def read_merge_sheet(file_obj, max_speaker_slot: int) -> List[Dict]:
    ...
    for row in rows:
        ...
        # each excel row is one speaker; SPEAKER_NAME_1 col always used per-row
        # since a row = one speaker regardless of final slot number assigned later
    ...
    for g in groups:
        if len(g["speakers"]) > max_speaker_slot and max_speaker_slot > 0:
            ...
            overflow_warnings.append(
                f"Group '{label}' has {len(g['speakers'])} speakers but the template "
                f"only supports {max_speaker_slot} speaker slot(s). Extra speakers will be dropped."
            )
